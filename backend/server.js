...
app.post('/api/residential/buildings-in-radius', async (req, res) => {
    ...
    try {
        ...
        // Load and parse the large JSON file
        const fs = require('fs');
        const path = require('path');
        
        // Read from both data files
        const data1Path = path.join(__dirname, 'data', 'data1.json');
        const data2Path = path.join(__dirname, 'data', 'data2.json');
        
        if (!fs.existsSync(data1Path) || !fs.existsSync(data2Path)) {
            return res.status(404).json({ error: 'Building data files not found. Expected data1.json and data2.json in backend/data/' });
        }

        // Read and combine both files
        const rawData1 = fs.readFileSync(data1Path, 'utf8');
        const rawData2 = fs.readFileSync(data2Path, 'utf8');
        const buildingsData1 = JSON.parse(rawData1);
        const buildingsData2 = JSON.parse(rawData2);
        
        // Combine both datasets
        const buildingsData = [...buildingsData1, ...buildingsData2];
        
        console.log(`Loaded ${buildingsData1.length} buildings from data_1.json and ${buildingsData2.length} buildings from data_2.json. Total: ${buildingsData.length} buildings`);
        ...
        // The JSON structure based on your example
        const buildings = Array.isArray(buildingsData) ? buildingsData : buildingsData.buildings || [];
            ...
            if (buildingLat && buildingLng && !isNaN(buildingLat) && !isNaN(buildingLng)) {
                ...
                if (distance <= maxDistance) {
                    buildingsInRadius.push({
                        ...
                        usage: building.Usage, // 'h' for house
                        operation: building.Operation, // 's' for sale
