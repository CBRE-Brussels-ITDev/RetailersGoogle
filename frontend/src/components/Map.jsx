...
const Map = forwardRef(({ onPlaceClick, onMapClick, selectedLocation, searchResults, searchResultsData, onClearAll, residentialMode = false }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    ...
    addRadiusCircle(center, radiusMeters) {
      addRadiusCircle(center, radiusMeters);
    },
  ...
  // Helper function to add radius circle
  const addRadiusCircle = async (center, radiusMeters) => {
    if (!mapView.current || !graphicsLayer.current) return;

    console.log(`Adding radius circle at ${center.lat}, ${center.lng} with radius ${radiusMeters}m`);

    try {
      const circlePolygon = await createCirclePolygon(center, radiusMeters);
      ...
      const circleSymbol = {
        type: 'simple-fill',
        color: [255, 0, 0, 0.1], // Light red with low opacity
        outline: {
          color: [255, 0, 0, 0.8], // Bright red outline
      ...
      graphicsLayer.current.add(circleGraphic);
      
      // Store reference for clearing
      if (!circleGraphics.current) {
        circleGraphics.current = [];
      }
      circleGraphics.current.push(circleGraphic);
  ...
  // Helper function to add building markers
  const addBuildingMarkers = async (buildings) => {
    if (!mapView.current || !graphicsLayer.current || !buildings.length) return;
    ...
    try {
      ...
      console.log(`Adding ${buildings.length} building markers to map`);
      ...
      for (const building of buildings) {
        ...
        const symbol = {
          type: 'simple-marker',
          color: building.price ? [255, 165, 0, 0.9] : [128, 128, 128, 0.9], // Orange for priced, gray for others
          outline: {
            color: [255, 255, 255, 1],
            width: 2
          },
          size: '10px' // Larger markers
        };

        const attributes = {
          ...building,
          displayName: building.address || `Building ${building.id}`,
          priceText: building.price ? `€${building.price.toLocaleString()}` : 'Price N/A',
          sizeText: building.size ? `${building.size}m²` : 'Size N/A'
        };
        ...
        graphicsLayer.current.add(graphic);
        buildingGraphics.current.push(graphic);
      }

      console.log(`Added ${buildings.length} building markers to map`);
  ...
  useEffect(() => {
    // Skip clearing markers in residential mode to preserve building markers and circles
    if (residentialMode) {
      return;
    }
    ...
    updateMarkers();
  }, [selectedLocation, searchResultsData, residentialMode]);
